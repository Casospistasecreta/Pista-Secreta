...
export function EvidenceToggle({
  item, accent, compact = false, className = "",
}: {
  item: EvidenceItem; accent: string; compact?: boolean; className?: string;
}) {
  ...
  return (
    <button
      type="button"
      onClick={(event) => { event.stopPropagation(); toggleEvidence(item); }}
      ...
      style={{ borderColor: marked ? accent : "var(--inv-line-strong)", color: marked ? accent : "var(--inv-dim)", background: marked ? `${accent}14` : "transparent" }}
      aria-pressed={marked}
    >
