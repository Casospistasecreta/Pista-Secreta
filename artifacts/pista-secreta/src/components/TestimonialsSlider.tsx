  ...
  return (
    <div className="flex gap-0.5 mb-3">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} className="text-primary/80 text-sm">★</span>
      ))}
    </div>
  );
}
        ...
        <div key={current} className="animate-in fade-in duration-500">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              <button
                onClick={prev}
                className="w-7 h-7 flex items-center justify-center border border-primary/20 rounded text-primary/50 hover:text-primary hover:border-primary/40 transition-colors font-serif"
              >
                ‹
              </button>
              <button
